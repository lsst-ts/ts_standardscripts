...
class TestEnableHexapodCompensationMode(
    standardscripts.BaseScriptTestCase, unittest.IsolatedAsyncioTestCase
):
    async def basic_make_script(self, index):
        self.script = EnableHexapodCompensationMode(index=index)
        self.script.mtcs = unittest.mock.AsyncMock()
        self.script.mtcs.start_task = utils.make_done_future()
        self.script.mtcs.enable_compensation_mode = unittest.mock.AsyncMock()
    ...
    async def make_dry_script(self):
        async with self.make_script():
            self.script.mtcs = unittest.mock.AsyncMock()
            self.script.mtcs.enable_compensation_for_hexapod = unittest.mock.AsyncMock()
            yield
    ...
    async def test_configure_with_program_reason(self):
        async with self.make_dry_script():
            ...
            assert (
                ...
                == "EnableHexapodCompensationMode BLOCK-123 202306060002 SITCOM-321"
            )
    ...
    async def test_executable(self):
        ...
        script_path = scripts_dir / "maintel" / "enable_hexapod_compensation_mode.py"
        print(script_path)
