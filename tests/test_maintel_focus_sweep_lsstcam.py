...
class TestFocusSweepLSSTCam(
    ...
    async def basic_make_script(self, index):
        self.script = FocusSweepLSSTCam(index=index)
    ...
    async def test_configure_focus_steps(self):
        config = {
            "axis": "z",
            "focus_steps": [0, 100, 200, 300, 400],
            "exp_time": 15.0,
            "filter": "g",
            "n_images_per_step": 1,
            "hexapod": "Camera",
        ...
        async with self.make_script():
            ...
            assert self.script.config.focus_steps == [-200, -100, 0, 100, 200]
            assert self.script.config.exp_time == 15.0
            assert self.script.config.filter == "g"
            assert self.script.config.n_images_per_step == 1
            assert self.script.hexapod == "Camera"

    async def test_configure_focus_steps_with_window(self):
        config = {
            "axis": "z",
            "focus_window": 400,
            "n_steps": 4,
        ...
        async with self.make_script():
            ...
            assert self.script.config.axis == "z"
            assert self.script.config.focus_window == 400
            assert self.script.config.n_steps == 4
            assert self.script.config.focus_steps == [-200, -100, 0, 100]
            assert self.script.config.exp_time == 15.0
            assert self.script.config.filter == "g"
            assert self.script.config.n_images_per_step == 1
            assert self.script.hexapod == "Camera"
    ...
    async def test_focus_sweep(self):
        ...
        async with self.make_script():
            ...
            assert self.script.lsstcam.take_focus.call_count == config["n_steps"]
    ...
    async def test_executable(self):
        ...
        script_path = scripts_dir / "maintel" / "focus_sweep_lsstcam.py"
