...
class TestOfflineComCam(standardscripts.BaseScriptTestCase, asynctest.TestCase):
                ...
                if getattr(self.script.group.check, comp):
                    ...
                    with self.subTest(f"{comp} summary state", comp=comp):
                        self.assertEqual(
                            ...
                            f"{comp}:  {current_state!r} != {salobj.State.OFFLINE!r}",
                        )

    async def test_executable(self):
        scripts_dir = standardscripts.get_scripts_dir()
        script_path = scripts_dir / "maintel" / "offline_comcam.py"
