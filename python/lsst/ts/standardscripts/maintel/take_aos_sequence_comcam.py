...
class TakeAOSSequenceComCam(BaseBlockScript):
    ...
    * sequence {n} of {m}: before taking a sequence.
    ...
    async def move_hexapod(self, target_z: float) -> None:
        ...
        ----------
        target_z : `float`
            Target z position to move to (relative offset).
        """
        z_move = target_z - self.current_z_position  # Calculate relative move
        await self.mtcs.offset_camera_hexapod(x=0, y=0, z=z_move, u=0, v=0)
